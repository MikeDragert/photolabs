...
module.exports = db => {
  ...
  //this route takes the search string, parsed it by spaces into seperate words,
  // and then looks for photos where one of the search text matches.  
  // ALL search words must be found in at least one of the available search texts
  router.get("/photos/:searchString", (request, response) => {
    const protocol = request.protocol;
    const host = request.hostname;
    const port = process.env.PORT || 8001;
    const serverUrl = `${protocol}://${host}:${port}`;
    ...
    const whereList = searchStrings.map((string, index) => {
      return (index > 0 ? 'AND ' : '') +
      `(LOWER(user_account.fullname) like $${index+1} OR 
        LOWER(user_account.username) like $${index+1} OR
        LOWER(photo.city) like $${index+1} OR
        LOWER(photo.country) like $${index+1} OR
        LOWER(topic.title) like $${index+1} )`;
    }).join('')

    const whereParams = searchStrings.map((string) => {
      return `%${string}%`;
    })
